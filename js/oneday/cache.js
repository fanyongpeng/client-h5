...
var Cache = (function(){
	...
	*/
	...
	Cache.prototype.updatePersonlist = function(list){
		if(!this.personlist[list.id]){
			this.personlist[list.id] = list;
		}else{
			for(var p in list){
				this.personlist[list.id][p] = list[p];
			}			
		}
	...
	Cache.prototype.getFriendAlias = function(account){
		for (var i = this.friendslist.length-1; i >= 0; i--) {
			if(this.friendslist[i].id == account){
	...
	Cache.prototype.getFriendslist = function(){
		...
		for(var i =0;i<this.friendslist.length; i++){
			array.push(this.getUserById(this.friendslist[i].id));
		}
		return array;
	};
	...
	Cache.prototype.getFriendslistOnShow = function(){
		var array = [];
		for(var i =0;i<this.friendslist.length; i++){
			if(!this.inBlacklist(this.friendslist[i].id)){
				array.push(this.getUserById(this.friendslist[i].id));
			}
		}
		return array;
	...
	*/
	...
	Cache.prototype.isTeamManager = function (account,id) {
		...
		if(obj){
			for (var i = obj.members.length - 1; i >= 0; i--) {
				if (obj.members[i].id === account&&(obj.members[i].type==='owner'||obj.members[i].type==='manager')) {
					return true
				} 
			};
		}
	...
	Cache.prototype.inBlacklist = function(account){
		for (var i = this.blacklist.length - 1; i >= 0; i--) {
			if(this.blacklist[i].id == account){
				return true;
			}
		};
		return false;
